...
describe('BeoordelingService', () => {
    ...
    test('creates a new beoordeling', async () => {
        const newBeoordeling = new Beoordeling({ beoordelingId: 1, score: 9, opmerkingen: 'Goed uitgevoerd', userId: 1 });
        (beoordelingDb.createBeoordeling as jest.Mock).mockResolvedValue(newBeoordeling);

        const result = await BeoordelingService.createBeoordeling(newBeoordeling);
        expect(result).toEqual(newBeoordeling);
    });

    test('deletes beoordeling by ID', async () => {
        (beoordelingDb.deleteBeoordelingById as jest.Mock).mockResolvedValue(true);

        const result = await BeoordelingService.deleteBeoordelingById(1);

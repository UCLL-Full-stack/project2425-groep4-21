...
describe('OpdrachtService', () => {
    ...
    test('creates a new opdracht', () => {
        ...
        (opdrachtDb.createOpdracht as jest.Mock).mockReturnValue(newOpdracht);

        const result = OpdrachtService.createOpdracht(newOpdracht);
        expect(result).toEqual(newOpdracht);
    });

    test('deletes opdracht by ID', () => {
        (opdrachtDb.deleteOpdrachtById as jest.Mock).mockReturnValue(true);

        const result = OpdrachtService.deleteOpdrachtById(1);
